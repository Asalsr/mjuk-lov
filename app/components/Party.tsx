 ...
 *  (homepage kit section, /kit landing). A first-class block, not a quiet card:
 *  the occasions are named and the CTA opens the same calm configurator. */
export function Party({ lang }: { lang: Lang }) {
  const t = ui[lang];
  return (
    <div
      className="px-6 py-[clamp(2.5rem,6vw,4.5rem)] md:px-12 flex flex-col md:flex-row md:items-center gap-8 md:gap-12"
      style={{ backgroundColor: "var(--vanilla-cream)", boxShadow: "0 4px 20px rgba(61, 42, 34, 0.05)" }}
      lang={lang}
    >
      <div className="max-w-[46rem] md:flex-1">
      ...
      {/* Fills the empty space beside the copy on wider screens. */}
