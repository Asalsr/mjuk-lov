...
export default async function Page({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const { data } = await db
    .from("orders")
    .select(
      "id, order_number, status, created_at, delivered_at, desired_date, fulfilment, address, dietary, notes, contact_name, contact_email, contact_phone, items, quoted_price, admin_note, product_name",
    )
